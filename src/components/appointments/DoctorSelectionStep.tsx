...
interface DocotorSelectionStep {
   selectedDentistId: string | null;
   onContinue:()=>void; 
   onSelectedDentist:(denstistId: string) => void;
}

const DoctorSelectionStep = ({selectedDentistId, onSelectedDentist, onContinue}:DocotorSelectionStep) => {
  const {data: dentists=[], isLoading} = useAvailableDoctors()

  if(isLoading) return (
  <div className='space-y-6'>
    <h1 className='text-2xl font-semibold'>Choose Your Dentist</h1>
    <DoctorCardsLoading/>
  </div>
  )
  return (
    <div className='space-y-6'>
      ...
      <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-6'>
        {dentists.map((dentist)=>(
          <Card key={dentist.id} className={`cursor-pointer transition-all hover:shadow-lg ${selectedDentistId === dentist.id ? "ring-2 ring-primary" : ""}`}
      ...
      </div>
      {
        selectedDentistId && (
          <div className='flex justify-end'>
            <Button onClick={onContinue}>Book Your Schedule</Button>
          </div>
        )
      }
    </div>
